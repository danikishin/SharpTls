...
| TM-06 | Certificate, hostname, EKU or CertificateVerify bypass | System/custom-root chain validation and time/EKU/name policy are secure defaults; bypass requires the explicit `DangerouslySkipServerCertificateValidation` opt-in and is documented as permitting active MITM. TLS CertificateVerify/server-key signatures and Finished remain mandatory even then. Signature schemes must match key OID/curve/PSS parameters and the exact TLS context. Generated PKI and platform interop tests cover both modes, wrong name, expiry, trust, EKU and signatures. |
| TM-07 | Ticket forgery, cross-origin or cross-validation-mode resumption, PSK index/hash confusion or replayed cache entry | Client tickets are single-offer owned, age/authentication capped and bound to origin, port, certificate-validation mode, ALPN, hash and ECH source. Server ticket state is AES-256-GCM authenticated or held in bounded zeroizing caches. Identity-specific binders, HRR reconstruction and selected-index checks precede resumed state exposure. |
...
- Online revocation availability and platform chain-policy differences remain deployment
  concerns. Unavailable evidence soft-fails by default without ignoring known revocation;
  deployments can require hard failure. `NoCheck` needs an explicit alternative policy.
